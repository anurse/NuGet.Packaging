...
namespace NuGet.ResolverTest
{
    public class ResolverTests
    {
        ...
        public void ResolveDependenciesForInstallDiamondDependencyGraph()
        {
            ...
            var packages = resolver.Resolve(target, sourceRepository).ToDictionary(p => p.Id);
        ...
        public void ResolveDependenciesForInstallDiamondDependencyGraphWithDifferentVersionsOfSamePackage()
        {
            ...
            // Act
            var resolver = new PackageResolver(DependencyBehavior.Lowest);
            var solution = resolver.Resolve(packageA, sourceRepository).ToArray();
        ...
        public void ResolveActionsPreferInstalledPackages()
        {
            ...
            var install = new List<ResolverPackage>() {
                CreatePackage("B", "1.0"),
                CreatePackage("C", "1.0"),
            };

            // Act
            var resolver = new PackageResolver(DependencyBehavior.HighestMinor, install);
            var solution = resolver.Resolve(target, sourceRepository).ToArray();
            var packages = solution.ToDictionary(p => p.Id);
        ...
        public void Resolver_Basic()
        {
            ...
            var resolver = new PackageResolver(DependencyBehavior.Lowest);
            var solution = resolver.Resolve(target, possible).ToList();
        ...
        public void Resolver_NoSolution()
        {
            ...
            var solution = resolver.Resolve(target, possible);
